...
class AccountVoucherWizard(models.TransientModel):
    ...
    def onchange_date(self):
        ...
        if self.journal_currency_id != self.currency_id:
            amount_advance = self.journal_currency_id._convert(
                self.amount_advance,
                self.currency_id,
                self.order_id.company_id or user.company_id,
    ...
    def _prepare_without_order_payment_vals(self):
    ...
    def action_view_payment(self, payment):
    ...
    def make_advance_payment(self):
        ...
        if sale_ids:
            ...
            payment_vals = self._prepare_without_order_payment_vals()
            payment = payment_obj.create(payment_vals)
            payment.action_post()
        ...
        return {
            ...
            'view_mode': 'tree,form',
            'res_model': 'account.payment',
            'views': [[self.env.ref('account.view_account_payment_tree').id or False, 'list'],[False, 'form']],
            'type': 'ir.actions.act_window',
            'domain': [('id', '=', payment.id)],
        }    
